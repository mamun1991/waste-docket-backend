...
export default async function uploadWasteFacilityRepSignature(
  _: any,
  {signatureData},
  context: any
) {
  ...
  try {
    ...
    if (err.name === 'JsonWebTokenError') {
      await createApiLog({
        ...
        functionName: 'uploadWasteFacilityRepSignature',
  ...
  try {
    ...
    const existingChunk = await WasteFacilityRepSignature.findOne({_id: signatureId});
    let uploadNewChunk;
    if (existingChunk) {
      ...
      uploadNewChunk = await WasteFacilityRepSignature.create({
