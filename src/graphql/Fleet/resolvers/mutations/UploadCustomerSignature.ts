...
export default async function uploadCustomerSignature(_: any, {signatureData}, context: any) {
  ...
  try {
    ...
    if (err.name === 'JsonWebTokenError') {
      await createApiLog({
        ...
        functionName: 'uploadCustomerSignature',
  ...
  try {
    ...
    const existingChunk = await CustomerSignature.findOne({_id: signatureId});
    let uploadNewChunk;
    if (existingChunk) {
      ...
      uploadNewChunk = await CustomerSignature.create({
