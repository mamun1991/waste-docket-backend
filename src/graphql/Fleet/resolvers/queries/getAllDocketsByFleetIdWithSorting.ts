...
export default async function getAllDocketsByFleetIdWithSorting(
  _: any,
  ...
  try {
    ...
    if (err.name === 'JsonWebTokenError') {
      console.log(`(getAllDocketsByFleetIdWithSorting) Invalid Permissions: Invalid JWT Token`);
      await createApiLog({
        type: ApiLogTypes.QUERY_END,
        level: ApiLogLevels.ERROR,
        functionName: 'getAllDocketsByFleetIdWithSorting',
        functionParams: [
          {
            paramName: 'fleetId',
            paramValue: fleetId,
    ...
    console.log('(getAllDocketsByFleetIdWithSorting) ', err.message);
    return {
      response: {
        message: err.message,
        status: 500,
  ...
  try {
    ...
    if (!user) {
      console.log(`(getAllDocketsByFleetIdWithSorting) User Not Found ${JSON.stringify(user)}`);
      await createApiLog({
    ...
    if (!fleet) {
      console.log(
        `(getAllDocketsByFleetIdWithSorting) User: ${JSON.stringify(
          user
    ...
    if (pageNumber && resultsPerPage) {
      ...
      console.log(`(getAllDocketsByFleetIdWithSorting) DocketData: ${JSON.stringify(DocketData)}`);
    } else {
      DocketData = await Docket.aggregate(query)
        .sort({
          [sortColumn]: sorts[sortOrder as 'asc' | 'desc'] as 1 | -1,
    ...
    console.log(`(getAllDocketsByFleetIdWithSorting) User: ${decodedToken.UserId} ${err.message}`);
    return {
      docketData: null,
      response: {
        message: err.message,
