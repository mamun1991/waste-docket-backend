...
export default async function getDocketsByFleetId(_: any, {fleetId, searchParams}, context: any) {
  ...
  try {
    ...
    if (err.name === 'JsonWebTokenError') {
      console.log(`(getDocketsByFleetId) Invalid Permissions: Invalid JWT Token`);
      await createApiLog({
        type: ApiLogTypes.QUERY_END,
        level: ApiLogLevels.ERROR,
        functionName: 'getDocketsByFleetId',
        functionParams: [
          {
            paramName: 'fleetId',
            paramValue: fleetId,
    ...
    console.log(`(getDocketsByFleetId) User: ${decodedToken.UserId}`, err.message);
    return {
      response: {
        message: err.message,
        status: 500,
  ...
  try {
    ...
    if (!user) {
      console.log(`(getDocketsByFleetId) User Not Found ${JSON.stringify(user)}`);
      await createApiLog({
    ...
    if (!fleet) {
      console.log(`(getDocketsByFleetId) Fleets not found ${JSON.stringify(fleet)}`);
      await createApiLog({
        type: ApiLogTypes.MUTATION_END,
        level: ApiLogLevels.ERROR,
        functionName: 'getDocketsByFleetId',
    ...
    if (!isOwnerOrMember) {
      console.log(`(getDocketsByFleetId) User is not a member or owner of this fleet`);
      await createApiLog({
        type: ApiLogTypes.MUTATION_END,
        level: ApiLogLevels.ERROR,
        functionName: 'getDocketsByFleetId',
    ...
    if (pageNumber && resultsPerPage) {
      ...
      console.log(
        `(getDocketsByFleetId) User: ${JSON.stringify(user)} DocketData: ${JSON.stringify(
          DocketData
        )}`
      );
    } else {
