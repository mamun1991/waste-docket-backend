...
export default async function getAllDocketsForAdmin(_: any, {searchParams}, context: any) {
  ...
  try {
    ...
    if (err.name === 'JsonWebTokenError') {
      await createApiLog({
        ...
        functionName: 'getAllDocketsForAdmin',
        functionParams: [
          {
  ...
  try {
    ...
    const {pageNumber, resultsPerPage, searchText, startDate, endDate} = searchParams;
    ...
    if (pageNumber && resultsPerPage) {
      UserData = await User.aggregate(query)
